 ...
 *
 * Copyright (C) 2013 , BTicino S.p.A. & Shidean
...
static void amico_gpio_configure(void)
{
	...
	gpio_configure_out(DM365_GPIO42, poSPK_Handle_PWR, 0,
		"Handle speaker power control");
	gpio_configure_out(DM365_GPIO42, poEN_Handle_MIC, 0,
	...
	if (amico_debug) {
		...
		gpio_export(poSPK_Handle_PWR, 0);
		gpio_export(poEN_Handle_MIC, 0);
	}
}
...
static struct platform_device amico_asoc_device[] = {
	{
		.name = "amico-p-asoc", /*Internal Voice codec + Zarlink*/
		.id = 0,
		.dev = {
...
MACHINE_START(AMICO_P, "Shidean amico-p board")
	.phys_io = IO_PHYS,
